...
public static class ExactCoverLevelGeneratorAlgorithm
{
    ...
    public static LevelData Generate(EditorGenerationRequest request)
    {
        ...
        for (int windowIndex = 0; windowIndex < candidateLengthWindows.Count; windowIndex++)
        {
            ...
            if (success)
            {
                ...
                levelData.notes = $"Generated by lazy exact-cover DAG generator. Seed: {context.Seed}. Candidate length window: {context.ActiveMinArrowLength}-{context.ActiveMaxArrowLength}. Generated candidates: {context.GeneratedCandidateCount}. Search frames: {context.SearchFrames}. Candidate checks: {context.CandidateChecks}.";
                return levelData;
            }
        ...
        {
            context.Fail("no exact cover found under current search limits");
        }
    ...
    private static GenerationContext BuildContext(EditorGenerationRequest request)
    {
        ...
        {
            Debug.LogWarning("Exact-cover generation failed: arrow count cannot be negative.");
            return null;
        }
        ...
        if (occupiedCells.Count == 0 && request.targetArrowCount != 0)
        {
            Debug.LogWarning("Exact-cover generation failed: occupied cells are empty but arrow count is not zero.");
            return null;
        }

        if (request.targetArrowCount == 0 && occupiedCells.Count != 0)
        {
            Debug.LogWarning("Exact-cover generation failed: arrow count is zero but occupied cells are not empty.");
            return null;
        }
        ...
        float adaptiveSeconds = request.gridSize >= 7 || request.targetArrowCount >= 10 ? 20f : 10f;
        ...
        return new GenerationContext
        {
            ...
            ArrowCount = request.targetArrowCount,
        ...
        HashSet<string> candidateKeys)
    {
        if (path.Count == 1)
        {
            ...
            {
                if (TryAddCandidate(context, path, Directions[i], candidates, candidateKeys))
                {
                    addedAny = true;
                }
            }
